...
// import air from '../../../public/img/hero/air.png'
...
export default function AirHeroSection() {
  const [tripType, setTripType] = useState("oneway");
  const [from, setFrom] = useState("");
  const [to, setTo] = useState("");
  const [journeyDate, setJourneyDate] = useState("");
  ...
  // ✅ UPDATED PROFESSIONAL AIR IMAGE
  const HERO_IMAGE =
    "https://flyingmag1.b-cdn.net/wp-content/uploads/sites/2/2022/07/shutterstock_559714906-scaled-1.jpg";
    ...
    <div className="mx-auto w-full max-w-[560px] md:max-w-[680px] lg:max-w-[860px] xl:max-w-7xl rounded-[28px] bg-white/95 p-4 shadow-[0_18px_50px_rgba(0,0,0,0.12)] ring-1 ring-black/5 backdrop-blur">
      ...
      <div className="grid grid-cols-1 gap-3 px-2 pb-2 xl:grid-cols-[1fr_auto_1fr_1.25fr_auto] xl:items-center">
        ...
        <button
          type="button"
          onClick={swap}
          className="mx-auto -my-1 flex h-10 w-10 xl:h-[56px] xl:w-[56px] items-center justify-center rounded-xl xl:rounded-2xl bg-transparent xl:bg-white hover:bg-gray-50"
        >
          <ArrowLeftRight className="h-6 w-6" />
        </button>
        ...
        <FieldShell
          ...
          icon={<CalendarDays className="h-5 w-5" />}
          rightAddon={
            <button className="text-[11px] xl:text-xs font-semibold text-orange-500">
              + ADD RETURN TRIP
            </button>
          }
        >
  ...
  return (
    <div className="w-full">
      {/* ✅ MOBILE / TABLET */}
      <section className="xl:hidden">
        <div className="relative h-[120px] md:h-[160px] lg:h-[180px] w-full overflow-hidden">
          <Image
            src={HERO_IMAGE}
            alt="Airplane hero"
            fill
      ...
      {/* ✅ DESKTOP XL+ */}
      <section className="relative hidden xl:block min-h-[500px] overflow-hidden">
        <div className="absolute inset-0">
          <Image
            src={HERO_IMAGE}
